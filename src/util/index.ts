...
export function loadEndpoints(endpoints: Map<string, Endpoint>) {
	...
	// Canvas
	endpoints.set('/canvas/gay', new Endpoint('/canvas/gay', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/glass', new Endpoint('/canvas/glass', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/wasted', new Endpoint('/canvas/wasted', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/triggered', new Endpoint('/canvas/triggered', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/greyscale', new Endpoint('/canvas/invert', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/invert', new Endpoint('/canvas/invert', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/invertgreyscale', new Endpoint('/canvas/invertgreyscale', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/brightness', new Endpoint('/canvas/brightness', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/threshold', new Endpoint('/canvas/threshold', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/sepia', new Endpoint('/canvas/sepia', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/red', new Endpoint('/canvas/red', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/green', new Endpoint('/canvas/green', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/blue', new Endpoint('/canvas/blue', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/color', new Endpoint('/canvas/color', {
		query: {
			avatar: { key: 'avatar', required: true },
			color: { key: 'color', required: true }
		}
	}));
	endpoints.set('/canvas/pixelate', new Endpoint('/canvas/pixelate', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/youtube-comment', new Endpoint('/canvas/youtube-comment', {
		query: {
			avatar: { key: 'avatar', required: true }
		}
	}));
	endpoints.set('/canvas/color-viewer', new Endpoint('/canvas/color-viewer', {
		query: {
			hex: { key: 'hex', required: true }
		}
	}));
	endpoints.set('/canvas/hex', new Endpoint('/canvas/hex', {
		query: {
			rgb: { key: 'rgb', required: true }
		}
	}));
	endpoints.set('/canvas/rgb', new Endpoint('/canvas/rgb', {
		query: {
			hex: { key: 'hex', required: true }
		}
	}));
	...
	// Other
	endpoints.set('/pokedex', new Endpoint('/pokedex', {
		query: {
			pokemon: { key: 'pokemon', required: true }
		}
	}));
	endpoints.set('/chatbot', new Endpoint('/chatbot', {
		cooldown: 4250, query: {
			message: { key: 'message', required: true }
		}
	}));
	endpoints.set('/mc', new Endpoint('/mc', {
		query: {
			username: { key: 'username', required: true }
		}
	}));
	endpoints.set('/lyrics', new Endpoint('/lyrics', {
		query: {
			title: { key: 'title', required: true }
		}
	}));
	endpoints.set('/binary', new Endpoint('/binary', {
		query: {
			text: { key: 'text' }, decode: { key: 'decode' }
		}
	}));
	endpoints.set('/base64', new Endpoint('/base64', {
		query: {
